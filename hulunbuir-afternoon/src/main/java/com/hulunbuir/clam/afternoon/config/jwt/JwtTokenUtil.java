package com.hulunbuir.clam.afternoon.config.jwt;
...
public final class JwtTokenUtil {
     ...
     * 初始化jwt密钥和过期时间
     *
     * --主要是用于在用户登录过后的信息存储，需要将生成的token返回给前端，并在每次请求的时候，在请求头添加此数据信息
     *
     * @author wangjunming
     ...
     */
    public String doUserInfoToken(LoginUser loginUser) {
     ...
     * @since 2020/3/23 12:54
     */
    public LoginUser getUserInfoByToken(String userToken) {
        return mapToEntity(getClaimFromToken(userToken).get("loginUser", LinkedHashMap.class), LoginUser.class);
    }
